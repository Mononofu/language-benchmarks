...
std::string VirtualPointToString(VirtualPoint p) {
  switch (p) {
    ...
    default: {
      ...
      return std::string(1, col) + std::to_string(row_col.first + 1);
    }
  }
}
...
VirtualPoint GoBoard::SingleLiberty(VirtualPoint p) const {
  ...
  CHECK_TRUE(IsInBoardArea(liberty));
  CHECK_TRUE(IsEmpty(liberty));
...
void GoBoard::SetStone(VirtualPoint p, GoColor c) {
  static const ZobristTable<uint64_t, kVirtualBoardPoints, 2>
      zobrist_values(
          /*seed=*/2765481);
...
VirtualPoint GoBoard::Chain::single_liberty() const {
  ...
  // liberties.
  CHECK_EQ(liberty_vertex_sum % num_pseudo_liberties, 0);
...
GoBoard CreateBoard(const std::string& initial_stones) {
  ...
  int row = 0;
  std::stringstream ss(initial_stones);
  for (std::string line; std::getline(ss, line, '\n');) {
    ...
    for (const auto& c : line) {
      if (c == ' ') {
        ...
        stones_started = true;
        CHECK_TRUE(board.PlayMove(VirtualPointFrom2DPoint({row, col}),
                                        GoColor::kBlack));
      } else if (c == 'O') {
        stones_started = true;
        CHECK_TRUE(board.PlayMove(VirtualPointFrom2DPoint({row, col}),
